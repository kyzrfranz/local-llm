...
let
  # Explicit GGUF path in the HF cache — validated by a manual llama-server run.
  # NOT an -hf repo:quant pin: the model and its mmproj live in two DIFFERENT HF
  # repos (see mmproj below), which -hf cannot express. Snapshot-hash-pinned —
  # see the fragility note on ExecStart.
  model    = "/var/lib/llama/hf/hub/models--AtomicChat--ornith-35b-GGUF/snapshots/677f2e37c48a09c38e006b8b9754ba810f4c0f22/ornith-35b-UD-Q4_K_XL.gguf";
...
in
{
  ...
  systemd.services.llama-serve = {
    description = "llama-server (Vulkan) — Node B single-model: Ornith-1.0-35B (dual-role)";
    ...
    serviceConfig = {
      ...
      # Vision projector (mmproj) for the multimodal 35B-A3B.
      # NOTE: pinned to a specific HF snapshot hash — if the model is ever re-pulled
      # or updated, this hash changes and the path breaks. Update it then, or
      # re-point via `find /var/lib/llama/hf -name '*mmproj*'`.
      # (Inlined literally in ExecStart below — a serviceConfig attr isn't in
      # scope inside the ExecStart string, which caused the undefined-var error.)
      ...
      ExecStart = ''
        ${llamaServer} \
          --model ${model} \
          --mmproj /var/lib/llama/hf/hub/models--bartowski--deepreinforce-ai_Ornith-1.0-35B-GGUF/snapshots/f9403b4da6306eb72fde0af1fe2df07cab1f88ce/mmproj-deepreinforce-ai_Ornith-1.0-35B-bf16.gguf \
