...
# Download one exact file as the service user, offline mode OFF. huggingface-cli
# is idempotent online: it skips a complete correct file, resumes a partial, and
# re-fetches a size/etag mismatch — i.e. "only fetch what's missing/wrong". Prints
# the resolved local path on stdout (last line); progress goes to stderr.
hf_get() {
  local repo="$1" file="$2"
  sudo -u "$LLAMA_USER" env -u HF_HUB_OFFLINE HF_HOME="$HF_HOME" \
    nix shell nixpkgs#huggingface-hub -c huggingface-cli download "$repo" "$file" | tail -n1
}
